...
def main():
    ...
    while True:      
        ...
        for event in pygame.event.get():#pen
            if event.type == pygame.QUIT:#escape from the programme
                return
            ...
            if event.type == pygame.MOUSEMOTION:#pen
                #if the mouse has moved, add a point to the list for the pen
            ...
            if event.type == pygame.MOUSEBUTTONDOWN: #rectangle
                ...
                if event.button == 1: 
                    ...
                    prevX1 =  event.pos[0]
                    prevY1 =  event.pos[1]
            #if isMouseDown falls, then we can end our drawing
            if event.type == pygame.MOUSEBUTTONUP:
                isMouseDown = False
...
def equilateraltriangle(x1, y1, x2, y2):
    side = min(abs(x2-x1), abs(y2-y1)) #weight
    height = round(side * 0.866) #height
    x = 1
    y = 1
    if (x2 > x1):
        x = -1
